...
export class SalleService {
  ...
  getAllSalles(search): Observable<Salle>{
    return this.http.get<Salle>(this.apiURL + '/salles/results?search_query=' + search)
    .pipe(
      retry(1)
    );  
  }
  ...
  updateSalle(id, salle): Observable<Salle> {
    return this.http.put<Salle>(this.apiURL + '/salle/' + id, JSON.stringify(salle), this.httpOptions)
    .pipe(
      retry(1),
      catchError(this.handleError)
    );
  ...
  deleteSalle(id) {
    return this.http.delete<Salle>(this.apiURL + '/salle/' + id, this.httpOptions)
    .pipe(
      retry(1),
      catchError(this.handleError)
    );
